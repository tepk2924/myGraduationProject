...
def callback(req: ExecutionRequest):
    ...
    pc_thresholded_world = ((camera_tf[:3, :3] @ pc_thresholded.T) + camera_tf[:3, 3:4]).T
    approaches_thresholded_world = (camera_tf[:3, :3] @ approaches_thresholded.T).T
    ...
    pc_filtered_msg.data = pc_thresholded_world.reshape((-1)).tolist()
    scores_filtered_msg.data = scores_thresholded.reshape((-1)).tolist()
    approaches_filtered_msg.data = approaches_thresholded_world.reshape((-1)).tolist()
...
if __name__ == "__main__":
    init()
    rospy.init_node("main_without_Unet")
