...
class Unet(tf.keras.models.Model):
    ...
    def train_step(self, data):
        # unpack data
        RGBE_normalized, gt_segmap_onehot = data #(B=1, H, W, 4), (B=1, H, W, 3)

        # get gradient
        with tf.GradientTape() as tape:
            # get network forward output
            logit = self(RGBE_normalized, training=True) #(B=1, H, W, 3)
    ...
    def test_step(self, data):
        # unpack data
        RGBE_normalized, gt_segmap_onehot = data #(H, W, 4), (H, W, 3)

        # get netwokr output
        logit = self(RGBE_normalized, training=False) #(H, W, 3)
...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        #B = 1
        return RGBE_normalized, gt_segmap_onehot #(B, H, W, 4), (B, H, W, 3)
