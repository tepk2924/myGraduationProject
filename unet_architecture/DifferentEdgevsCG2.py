...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        kernaled = (signal.convolve2d(colors[:, :, 0], kernal_sober_x, mode="same", boundary="symm")**2 + signal.convolve2d(colors[:, :, 0], kernal_sober_y, mode="same", boundary="symm")**2 + 
                    signal.convolve2d(colors[:, :, 1], kernal_sober_x, mode="same", boundary="symm")**2 + signal.convolve2d(colors[:, :, 1], kernal_sober_y, mode="same", boundary="symm")**2 + 
                    signal.convolve2d(colors[:, :, 2], kernal_sober_x, mode="same", boundary="symm")**2 + signal.convolve2d(colors[:, :, 2], kernal_sober_y, mode="same", boundary="symm")**2)
